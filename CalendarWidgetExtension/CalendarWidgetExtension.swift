                    ...
                    LazyVGrid(columns: columns, spacing: 7) {
                        ForEach(entry.days) { day in
                            Text(day.date!.formatted(.dateTime.day()))
                                ...
                                .foregroundColor(day.done ? .teal : .secondary)
                                .background(
                                    Circle()
                                        .foregroundColor(.teal.opacity(day.done ? 0.3 : 0.0))
                                        .scaleEffect(1.4)
                                )
                        }
                    }
...
private struct LockScreenCircularView: View {
    ...
    var body: some View {
        Gauge(value: Double(currentStreak), in: 0...Double(entry.days.count)) {
